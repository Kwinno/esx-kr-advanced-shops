...
$(function() {
		window.addEventListener('message', function(event) {
			...
			$('.image').on('click', function () {
				...
				$.post('http://esx-advanced-fuel-shops/putcart', JSON.stringify({item: item, price : price, label : label, count : count, id : id}), function( cb ) {
			...
			$("body").on("click", "#refreshcart", function() {
				$.post('http://esx-advanced-fuel-shops/escape', JSON.stringify({}));
				location.reload(true);
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
				$.post('http://esx-advanced-fuel-shops/refresh', JSON.stringify({}));
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$('#payment').hide("fast");
						$('#cart').hide("fast");
						$.post('http://esx-advanced-fuel-shops/buy', JSON.stringify({Count : value[i].value, Item : value[i].id}));
					} 
					else {
						$.post('http://esx-advanced-fuel-shops/notify', JSON.stringify({msg : "~r~One of the item does not have enough stock or the amount is invalid."}));
					}
				}
			});
			...
			$("body").on("click", "#bossactions", function() {
				$.post('http://esx-advanced-fuel-shops/bossactions', JSON.stringify({}));
				$.post('http://esx-advanced-fuel-shops/escape', JSON.stringify({}));
				location.reload(true);
				$.post('http://esx-advanced-fuel-shops/emptycart', JSON.stringify({}));
				$('#wrapper').hide("fast");
				$('#payment').hide("fast");
				$('#cart').hide("fast");
			});
